...
public class CameraFilterToneCurve extends CameraFilter {
    ...
    protected int createProgram(Context applicationContext) {
        return GlUtil.createProgram(applicationContext, R.raw.vertex_shader, R.raw.fragment_shader_ext_tone_curve);
    }
        ...
        if ((mRedCurve.size() >= 256) && (mGreenCurve.size() >= 256) && (mBlueCurve.size() >= 256)
            ...
            for (int currentCurveIndex = 0; currentCurveIndex < 256; currentCurveIndex++) {
                ...
                toneCurveByteArray[currentCurveIndex * 4
                        + 2] = (byte) ((int) Math.min(Math.max(currentCurveIndex + mBlueCurve.get(currentCurveIndex)
                                + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
                toneCurveByteArray[currentCurveIndex * 4
                        + 1] = (byte) ((int) Math.min(Math.max(currentCurveIndex + mGreenCurve.get(currentCurveIndex)
                                + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
                toneCurveByteArray[currentCurveIndex
                        * 4] = (byte) ((int) Math.min(Math.max(currentCurveIndex + mRedCurve.get(currentCurveIndex)
                                + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
    ...
    protected void bindGLSLValues(float[] mvpMatrix, FloatBuffer vertexBuffer, int coordsPerVertex, int vertexStride,
        ...
        // GLES20.glVertexAttribPointer(maExtraTextureCoordLoc, 2, GLES20.GL_FLOAT,
        // false, texStride,
        // texBuffer);
    }
    ...
    private ArrayList<Float> createSplineCurve(PointF[] points) {
        ...
        // If we have a first point like (0.3, 0) we'll be missing some points at the
        // beginning
        // that should be 0.
    ...
    private ArrayList<Double> createSecondDerivative(Point[] points) {
        ...
        matrix[n - 1][1] = 1;
        // What about matrix[n-1][0] and matrix[n-1][2]? For now, assuming they are 0
        // (Brad L.)
        matrix[n - 1][0] = 0;
        matrix[n - 1][2] = 0;
